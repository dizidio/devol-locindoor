...
class GenomeHandler:
    ...
    def generate2(self, size):
        genome_all = []
        lhs_result = lhs(self.convolution_layers * self.convolution_layer_size \
                        + self.dense_layers * self.dense_layer_size + 1, size);
        for individual in range(size):
            genome = [];
            ind_lhs = lhs_result[individual];
            #for i in range(self.convolution_layers):
            for ind,key in zip(range(self.convolution_layer_size*self.convolution_layers), self.convolutional_layer_shape*self.convolution_layers):
                index = np.round(ind_lhs[ind]*(len(self.layer_params[key])-1));
                param = self.layer_params[key]
                genome.append(param[int(index)])
      
            
            #for i in range(self.dense_layers):
            for ind,key in zip(range(self.dense_layer_size*self.dense_layers), self.dense_layer_shape*self.dense_layers):
                ind = ind + self.convolution_layers * self.convolution_layer_size;
                index = np.round(ind_lhs[ind]*(len(self.layer_params[key])-1));
                param = self.layer_params[key]
                genome.append(param[int(index)])
                    
            index = np.round(ind_lhs[-1]*(len(self.optimizer)-1))
            genome.append(list(range(len(self.optimizer)))[int(index)])
            genome[0] = 1
            genome_all.append(genome);
        #print(genome_all);
        return genome_all
